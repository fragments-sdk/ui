...
import { handleArrowNavigation, A11yVisuallyHidden } from "./a11y";
...
describe("handleArrowNavigation", () => {
  ...
  it("moves down with ArrowDown in vertical orientation", () => {
    expect(
      handleArrowNavigation(makeEvent("ArrowDown"), items, 0, { orientation: "vertical" })
    ).toBe(1);
  });

  it("moves up with ArrowUp in vertical orientation", () => {
    expect(handleArrowNavigation(makeEvent("ArrowUp"), items, 1, { orientation: "vertical" })).toBe(
      0
    );
  });
  ...
  it("ignores ArrowDown in horizontal orientation", () => {
    expect(
      handleArrowNavigation(makeEvent("ArrowDown"), items, 0, { orientation: "horizontal" })
    ).toBeUndefined();
  });

  it("moves right with ArrowRight in horizontal orientation", () => {
    expect(
      handleArrowNavigation(makeEvent("ArrowRight"), items, 0, { orientation: "horizontal" })
    ).toBe(1);
  });

  it("loops from last to first when loop is enabled", () => {
    expect(
      handleArrowNavigation(makeEvent("ArrowDown"), items, 2, {
  ...
  it("handles both orientation (vertical + horizontal keys)", () => {
    expect(handleArrowNavigation(makeEvent("ArrowDown"), items, 0, { orientation: "both" })).toBe(
      1
    );
    expect(handleArrowNavigation(makeEvent("ArrowRight"), items, 0, { orientation: "both" })).toBe(
      1
    );
  });
...
describe("VisuallyHidden (utils)", () => {
  it("hides content visually with inline styles", () => {
    render(<A11yVisuallyHidden>SR only</A11yVisuallyHidden>);
  ...
  it("does not apply hidden styles when focusable", () => {
    render(<A11yVisuallyHidden focusable>Skip link</A11yVisuallyHidden>);
  ...
  it("sets data-visually-hidden attribute when not focusable", () => {
    render(<A11yVisuallyHidden>Hidden</A11yVisuallyHidden>);
    expect(screen.getByText("Hidden")).toHaveAttribute("data-visually-hidden", "true");
  });

  it("has no accessibility violations", async () => {
    const { container } = render(<A11yVisuallyHidden>Accessible</A11yVisuallyHidden>);
