...
const ACCEPTED_FROZEN_PROFILE_HASHES = {
  ...
  legacy: "ab27f9e4a29d0d44720da2dc8a1613f619fee868a562658bdfbbc84bfe88256d",
};
...
export function validateMeasurements(measurements) {
  ...
  assertRecord(measurements.legacy.navigation, "legacy.navigation");
...
export async function renderScss(measurements, source) {
  ...
  const compatibilityAliases = [
    ...
    "  --fui-input-height-sm: var(--fui-control-height-sm);",
    "  --fui-input-height: var(--fui-control-height-md);",
    "  --fui-input-height-lg: var(--fui-control-height-lg);",
  ].join("\n");
  const outputWithScopedAliases = output
...
function cssPxNumber(value, path) {
  const match = /^([0-9]+(?:\.[0-9]+)?)px$/.exec(value);
  if (!match) fail(`${path} must be a positive px length`);
  return Number(match[1]);
}

function docsDensityProperties(profile, spacingMultipliers, defaultDensity, targetSizeMinimum) {
  const rem = (value) => `${value / profile.baseFontSize}rem`;
  const unitRem = profile.baseUnit / profile.baseFontSize;
  const defaultBaseFontSize = cssPxNumber(
    defaultDensity.baseFontSize,
    "density.default.baseFontSize"
  );
  const properties = {
  ...
  for (const [size, value] of Object.entries(defaultDensity.controlHeight)) {
    properties[`--fui-control-height-${size}`] = `${
      cssPxNumber(value, `density.default.controlHeight.${size}`) / defaultBaseFontSize
    }rem`;
  }
  properties["--fui-target-size-min"] = targetSizeMinimum;
  return {
    ...properties,
    "--fui-button-height-sm": rem(profile.buttonHeights[0]),
    "--fui-button-height-md": rem(profile.buttonHeights[1]),
    "--fui-button-height-lg": rem(profile.buttonHeights[2]),
    "--fui-input-height-sm": rem(profile.inputHeights[0]),
    "--fui-input-height": rem(profile.inputHeights[1]),
    "--fui-input-height-lg": rem(profile.inputHeights[2]),
